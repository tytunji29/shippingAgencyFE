...
export default function QuoteDataTable() {
  ...
  const handleAcceptBid = async (quote: Bid) => {
    const tenPercent = Number(quote.amount) * 0.1;
    const formattedTotal = formatNaira(tenPercent);

    const confirmed = await Swal.fire({
      title: "Accept this bid?",
      html: `A total of <strong>${formattedTotal}</strong> will be paid (10% of the quote amount).`,
      icon: "warning",
    ...
    if (confirmed.isConfirmed) {
      try {
        ...
        if (res.data.status) {
          ...
          toast.fire({ icon: "error", title: res.data.message || "Failed to accept" });
        }
      } catch (err) {
    ...
    <div className="max-w-6xl mx-auto mb-10 lg:p-4 bg-white">
            ...
            {paginatedShipments.map((shipment) => {
              ...
              return (
                <tr key={shipment.shipmentId} className="hover:bg-gray-50">
                  ...
                  <td className="px-2 py-4 text-sm text-gray-500">{formatNaira(shipment.quote)}</td>
                  ...
                  <td className="px-2 py-4">
                    <Button
                      ...
                      onClick={() => {
                        ...
                        if (mainUser === "Yes") {
                          if (shipment.asBidded !== "Yes") setShowBidModal(true);
                        } else {
      ...
      {totalPages > 1 && (
        <div className="flex justify-center gap-4 mt-6">
          ...
          <span className="text-sm pt-1">Page {currentPage} of {totalPages}</span>
          ...
          {selectedShipment?.quotes?.length ? (
            <div className="space-y-2 max-h-80 overflow-y-auto">
              {selectedShipment.quotes.map((q) => (
                <div key={q.quoteId} className="flex justify-between items-center border p-2 rounded">
                  <div>
                    <p className="text-sm"><strong>Amount:</strong> {formatNaira(q.amount)}</p>
                    <p className="text-xs text-gray-500">Submitted: {q.dateSubmitted && formatDate(q.dateSubmitted)}</p>
                  </div>
                  <Button size="sm" onClick={() => handleAcceptBid(q)}>Accept</Button>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-sm text-gray-600">No bids yet for this shipment.</p>
          )}
        </DialogContent>
      </Dialog>
    </div>
